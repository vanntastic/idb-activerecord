...
// Browser uses TursoAdapter in HTTP mode to talk to the proxy at localhost:3002.
// The proxy uses TursoAdapter with a raw libsql client to sync with the remote
// Turso database.
...
const db = new Database('turso-sync-demo');
...
async function init() {
  ...
  try {
    ...
    logAction(`⚠️ Could not connect to ${API_URL}. Run: npm run example:turso-api`);
    console.error(err);
  }
